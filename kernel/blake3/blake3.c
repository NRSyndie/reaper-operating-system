...
INLINE size_t compress_chunks_parallel(const uint8_t *input, size_t input_len,
                                       ...
                                       uint8_t *out) {
  const uint8_t *chunks_array[MAX_SIMD_DEGREE_OR_2];
  size_t input_position = 0;
  size_t chunks_array_len = 0;
  while (input_len - input_position >= BLAKE3_CHUNK_LEN && chunks_array_len < MAX_SIMD_DEGREE) {
    chunks_array[chunks_array_len] = &input[input_position];
    input_position += BLAKE3_CHUNK_LEN;
    chunks_array_len += 1;
  }

  if (chunks_array_len > 0) {
...
INLINE size_t compress_parents_parallel(const uint8_t *child_chaining_values,
  ...
  while (num_chaining_values - (2 * parents_array_len) >= 2 && parents_array_len < MAX_SIMD_DEGREE_OR_2) {
    parents_array[parents_array_len] =
        &child_chaining_values[2 * parents_array_len * BLAKE3_OUT_LEN];
    parents_array_len += 1;
  }

  if (parents_array_len > 0) {
