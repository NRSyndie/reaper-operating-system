...
echo "[day16] building kernel iso"
make -C kernel iso >/dev/null

echo "[day16] running matrix with runs=${RUNS} timeout=${TIMEOUT_SECS}s"
./tools/run_law2_fate_matrix.sh --runs "${RUNS}" --timeout "${TIMEOUT_SECS}" --iso "${ISO_PATH}" --out-dir "${OUT_DIR}"

echo "[day16] validating Day 16 markers across all runs"
for ((i = 1; i <= RUNS; i++)); do
    ...
    for marker in \
        ...
        "[TEST] Day 16 Unmap/Remap Contract: SUCCESS."; do
        if ! grep -Fq "${marker}" "${serial_file}"; then
            echo "ERROR: run ${i} missing Day 16 marker: ${marker}" >&2
            exit 1
        fi
    done

    if grep -Fq "[DAY16-FAIL]" "${serial_file}"; then
        echo "ERROR: run ${i} contains Day 16 forbidden marker: [DAY16-FAIL]" >&2
        exit 1
    fi

    echo "[day16] run ${i} PASS (${serial_file})"
done

echo "[day16] PASS: Day 16 closure suite succeeded for ${RUNS}/${RUNS} runs"
