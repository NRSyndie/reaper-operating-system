...
echo "[day28] building kernel iso"
make -C "${REPO_ROOT}/kernel" iso >/dev/null

echo "[day28] running matrix with runs=${RUNS} timeout=${TIMEOUT_SECS}s"
"${REPO_ROOT}/tools/run_law2_fate_matrix.sh" --runs "${RUNS}" --timeout "${TIMEOUT_SECS}" --iso "${ISO_PATH}" --out-dir "${OUT_DIR}"

echo "[day28] validating Day 28 markers across all runs"
for ((i = 1; i <= RUNS; i++)); do
    ...
    for marker in \
        ...
        "[TEST] Day 28 Strict Chain Contract: SUCCESS."; do
        if ! grep -Fq "${marker}" "${serial_file}"; then
            echo "ERROR: run ${i} missing Day 28 marker: ${marker}" >&2
            exit 1
        fi
    done
    ...
    echo "[day28] run ${i} PASS (${serial_file})"
done

echo "[day28] PASS: Day 28 closure suite succeeded for ${RUNS}/${RUNS} runs"
