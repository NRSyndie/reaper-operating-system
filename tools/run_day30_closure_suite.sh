...
echo "[day30] building kernel iso"
make -C "${REPO_ROOT}/kernel" iso >/dev/null

echo "[day30] running matrix with runs=${RUNS} timeout=${TIMEOUT_SECS}s"
"${REPO_ROOT}/tools/run_law2_fate_matrix.sh" --runs "${RUNS}" --timeout "${TIMEOUT_SECS}" --iso "${ISO_PATH}" --out-dir "${OUT_DIR}"

echo "[day30] validating Day 30 markers across all runs"
for ((i = 1; i <= RUNS; i++)); do
    ...
    echo "[day30] run ${i} PASS (${serial_file})"
done

echo "[day30] PASS: Day 30 closure suite succeeded for ${RUNS}/${RUNS} runs"
