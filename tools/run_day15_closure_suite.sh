...
echo "[day15] building kernel iso"
make -C kernel iso >/dev/null

echo "[day15] running matrix with runs=${RUNS} timeout=${TIMEOUT_SECS}s"
./tools/run_law2_fate_matrix.sh --runs "${RUNS}" --timeout "${TIMEOUT_SECS}" --iso "${ISO_PATH}" --out-dir "${OUT_DIR}"

echo "[day15] validating Day 15 markers across all runs"
for ((i = 1; i <= RUNS; i++)); do
    ...
    for marker in \
        ...
        "PARADIGM: Genesis bridge probe PASS."; do
        if ! grep -Fq "${marker}" "${serial_file}"; then
            echo "ERROR: run ${i} missing Day 15 marker: ${marker}" >&2
            exit 1
        fi
    done

    for bad in "[DAY15-FAIL]" "PARADIGM: Genesis bridge probe FAIL."; do
        if grep -Fq "${bad}" "${serial_file}"; then
            echo "ERROR: run ${i} contains Day 15 forbidden marker: ${bad}" >&2
            exit 1
        fi
    done

    echo "[day15] run ${i} PASS (${serial_file})"
done

echo "[day15] PASS: Day 15 closure suite succeeded for ${RUNS}/${RUNS} runs"
