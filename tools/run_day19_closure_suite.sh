...
echo "[day19] building kernel iso"
make -C kernel iso >/dev/null

echo "[day19] running matrix with runs=${RUNS} timeout=${TIMEOUT_SECS}s"
./tools/run_law2_fate_matrix.sh --runs "${RUNS}" --timeout "${TIMEOUT_SECS}" --iso "${ISO_PATH}" --out-dir "${OUT_DIR}"

echo "[day19] validating Day 19 markers across all runs"
for ((i = 1; i <= RUNS; i++)); do
    ...
    for marker in \
        ...
        "[TEST] Day 19 Mint Monotonicity: SUCCESS."; do
        if ! grep -Fq "${marker}" "${serial_file}"; then
            echo "ERROR: run ${i} missing Day 19 marker: ${marker}" >&2
            exit 1
        fi
    done

    if grep -Fq "[DAY19-FAIL]" "${serial_file}"; then
        echo "ERROR: run ${i} contains Day 19 forbidden marker: [DAY19-FAIL]" >&2
        exit 1
    fi

    echo "[day19] run ${i} PASS (${serial_file})"
done

echo "[day19] PASS: Day 19 closure suite succeeded for ${RUNS}/${RUNS} runs"
