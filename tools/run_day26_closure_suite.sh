...
echo "[day26] building kernel iso"
make -C kernel iso >/dev/null

echo "[day26] running matrix with runs=${RUNS} timeout=${TIMEOUT_SECS}s"
./tools/run_law2_fate_matrix.sh --runs "${RUNS}" --timeout "${TIMEOUT_SECS}" --iso "${ISO_PATH}" --out-dir "${OUT_DIR}"

echo "[day26] validating Day 26 markers across all runs"
for ((i = 1; i <= RUNS; i++)); do
    ...
    for marker in \
        ...
        "[TEST] Day 26 Attunement Contract: SUCCESS."; do
        if ! grep -Fq "${marker}" "${serial_file}"; then
            echo "ERROR: run ${i} missing Day 26 marker: ${marker}" >&2
            exit 1
        fi
    done

    if grep -Fq "[DAY26-FAIL]" "${serial_file}"; then
        echo "ERROR: run ${i} contains Day 26 forbidden marker: [DAY26-FAIL]" >&2
        exit 1
    fi

    echo "[day26] run ${i} PASS (${serial_file})"
done

echo "[day26] PASS: Day 26 closure suite succeeded for ${RUNS}/${RUNS} runs"
